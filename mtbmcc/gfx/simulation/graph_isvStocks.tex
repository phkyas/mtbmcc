...
\begin{tikzpicture}[x=0.1cm,y=0.05cm]
  ...
  \draw[->] (\xmin,\ymin) -- (\xmin,\ymax) node[above] {\footnotesize $Independent~Software~Vendor~Stocks~in~\%$};
  ...
  %,mark=*,mark size=0.5pt
	\draw[color=red] plot[smooth] file {simulationData/ISV/potentialISV.txt}
			node [above=0.3cm, right] {\scriptsize $Potential~ISV$};
	\draw[color=blue] plot[smooth] file {simulationData/ISV/populationISV.txt}
			node [right] {\scriptsize $Population~ISV$};	

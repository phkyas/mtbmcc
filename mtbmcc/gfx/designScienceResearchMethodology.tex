...
\node[draw,text width=8em,text centered, rectangle,rounded corners,minimum height=4em,thick,rotate=90] (a1) {
	\begin{minipage}{8em}\centering
		...
		~\\~\\~\\~\\~\\
		(cf. Section \ref{ch:intro:mo}; addresses \ac{DSR} G2)
		\\~\\
	\end{minipage}
};
...
\begin{minipage}{8em}\centering
		...
		~\\~\\
		(cf. Chapter \ref{ch:sota}, \ref{ch:cld}, and \ref{ch:sfd}; addresses \ac{DSR} G1, G4-G6)
		\\~\\
	\end{minipage}
};
...
\begin{minipage}{8em}\centering
		...
		~\\~\\~\\~\\~\\~\\~\\~\\~\\~\\
		(cf. Section \ref{ch:sota:cm} and Chapter \ref{ch:cld}-\ref{ch:poc}; addresses \ac{DSR} G3)
	\end{minipage}
};
